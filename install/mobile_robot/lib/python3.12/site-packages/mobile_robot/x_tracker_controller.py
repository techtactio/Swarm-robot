...
class XAxisTrackerAndCleaner(Node):
    ...
    def robot_count_callback(self, msg):
        if msg.data > self.robot_count:
            ...
            self.partition_min_x = self.map_length / float(self.robot_count) if self.map_length else 0.0
    ...
    def turn_in_place(self, target_yaw):
        ...
        if abs(diff) > 0.05:
            ...
            cmd.angular.z = max(-0.5, min(0.5, 1.0 * diff))
    ...
    def move_backward(self, distance):
        ...
        if math.sqrt(dx*dx + dy*dy) < distance: cmd = Twist(); cmd.linear.x = -0.2; self.cmd_pub.publish(cmd); return False
    ...
    def wait_in_place(self, duration):
        ...
        if (self.get_clock().now() - self.wait_start_time).nanoseconds / 1e9 < duration: self.cmd_pub.publish(Twist()); return False
        self.wait_start_time = None; return True
    ...
    def lidar_callback(self, scan):
        ...
        if self.state == "MEASURING":
            if self.current_front_dist < 0.6:
                ...
                return
            right = ranges[self.right_idx]; cmd = Twist(); error = 0.5 - right; cmd.linear.x = 0.5; cmd.angular.z = 1.5 * error; self.cmd_pub.publish(cmd)
            ...
            if self.approaching_waste:
                ...
                if self.current_waste_distance > 0.4:
                    cmd = Twist(); cmd.linear.x = 0.2; self.cmd_pub.publish(cmd)
                else:
                    self.get_logger().info("WASTE REMOVED/COLLECTED")
                    if self.detected_wastes: self.detected_wastes.pop()
                    self.approaching_waste = False; self.current_waste_distance = None; self.stop_robot()
                return

            if not self.is_turning: self.scan_for_waste(scan, ranges)
    ...
    def scan_for_waste(self, scan, ranges):
        ...
        for i in range(start, end):
            ...
            if 0.3 < dist < self.WASTE_DETECT_RANGE:
                ...
                # === R2 WALL FILTERS (Adapted Sample Logic) ===
                # Ignore Partition Start (R1 Zone)
                if waste_x < (self.partition_min_x - 0.5): continue

                # Ignore End Wall (using map length if available)
                # Assumes Start X is approx -10, Length is 20, End is +10.
                # Or if local X > length. Let's use Local X check for End Wall 
                # because we know length is relative to start.
                local_angle = self.current_yaw + angle
                w_local_x = self.current_x + dist * math.cos(local_angle)
                if self.map_length is not None:
                    if w_local_x > (self.map_length - 0.5): continue

                # Ignore Side Walls
                if self.map_width is not None:
                     if waste_y < 0.5 or waste_y > (self.map_width - 0.5): continue
                ...
                if is_new:
                    self.detected_wastes.append((waste_x, waste_y))
                    self.get_logger().info(f"NEW GLOBAL WASTE: ({waste_x:.2f}, {waste_y:.2f})")
                break
    ...
    def run_cleaning_fsm(self):
        if self.clean_step == 0:
            ...
            if self.current_x <= self.partition_min_x: self.stop_robot(); self.get_logger().info("R2 Done."); return
            ...
            else: self.step_start_pos = 0.0; self.fixed_axis_value = 0.0; self.clean_step = 5
            ...
            if self.turn_in_place(math.pi): self.clean_step = 10
             ...
             if abs(self.current_x - self.step_start_pos) < 2.0: 
                 self.move_straight_locked(math.pi, 'y', self.fixed_axis_value)
             else: self.step_start_pos = 0.0; self.fixed_axis_value = 0.0; self.clean_step = 11
